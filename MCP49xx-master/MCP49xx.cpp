...
MCP49xx::MCP49xx(MCP49xx::Model _model, int _ss_pin, int _LDAC_pin) : bufferVref(false), gain2x(false), port_write(false), spi_divider(SPI_CLOCK_DIV2), automaticallyLatchDual(true)
{
  this->ss_pin = _ss_pin;
...
void MCP49xx::setPortWrite(boolean _port_write)
{
...
boolean MCP49xx::setSPIDivider(int _div)
{
  switch (_div)
  {
  case SPI_CLOCK_DIV2:
