...
public:
    ...
    static constexpr float sampling_frequency = 240000;
    static constexpr float sampleRateMicros = 1 / float(sampling_frequency) * 1000000;

    static const int maximumSymbolsIn = 9000; // 16384;
    static const int numberOfGThreads = 3;
    static const int sampleDelay = 250;
    int latestActiveMessage = 0;
    static const int messageNumberToSwitchToDelay = 1;

    static constexpr float defaultEndMessageThreshold = 50;
    ...
    */
    static const int number_of_symbols = 1;
    static const int number_of_fh_pairs = 1;
    // should change to once frequency per line
    float fhfsk_data[number_of_fh_pairs][number_of_symbols * 4] = {{120000, 0, 0, 1}};//0.65, 0.45
    static const int plusMinusBins = 3; // how many nearby bins to look at for frequency
...
private:
    ...
    const int startMessageThresholds[numberOfGThreads] = {2000, 2000, 2000};//, 14050, 15250, 16450}; //{4450, 5650, 6850, 7050, 8250, 9450}; //{100, 140, 180, 220, 250, 300, 350, 400, 450}; //{120, 200, 250, 300, 350, 400, 450, 500, 550, 600}; // 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 2000, 2500, 3000, 3500, 4000, 4500, 5000};
    ...
    static const int meanTrackerSize = 6;
