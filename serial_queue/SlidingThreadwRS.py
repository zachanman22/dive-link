...
class SlideWinThread(threading.Thread):
    def __init__(self, received_queue=None, to_transmit_queue=None, fs=400000, bitrate = 200, messageSize = 128, eccBytes = 0):
        super().__init__()
        self.received_queue = received_queue
        self.to_transmit_queue = to_transmit_queue
        ...
        self.status = self.INITIALIZED
        self.should_stop = False
        self.ecc_bytes = eccBytes
        self.rs = rs.RSCodec(eccBytes)
        # print("Connected to serial port ", port)
    ...
    def goertzel2bits(self, transmission):
        ...
        bit = 0
        #array of results
        message = [0 for i in range(self.messageSize//4)]
        #window size 
        ...
        for i in range(self.ecc_bytes):
            message[-1-i] = message[-2-i]
        message[-2-self.ecc_bytes] = 0
        try:
            print(message)
            output = self.rs.decode(bytearray(message))[0]
            output = output.decode("utf-8")
            self.to_transmit_queue.put(output)
        except:
            self.to_transmit_queue.put("Message Decode Failed, Too Much Noise")
...
if __name__ == "__main__":
    external_serial_thread_port = "COM12"
    ...
    window_thread = SlideWinThread(
        received_queue=from_external_data_queue, to_transmit_queue = to_RS_queue, fs=400000, bitrate=200, messageSize=64, eccBytes = 4)
    window_thread.start()
